...
local ELECTRONICS = function()
	local socModeSection = "SOC_MODE"
	local socModeLast = car.mgukRecovery
	local deployModeSection = "DEPLOY_MODE"
	local deployModeLast = car.mgukDelivery
	local mguhModeSection = "MGUH_MODE"
	local mguhModeLast = car.mguhChargingBatteries

	return function()
		if sim.isInMainMenu then
			ac.setScriptSetupValue(socModeSection, car.mgukRecovery)
			ac.setScriptSetupValue(deployModeSection, car.mgukDelivery)
			ac.setScriptSetupValue(mguhModeSection, car.mguhChargingBatteries and 1 or 0)
			socModeLast = car.mgukRecovery
			deployModeLast = car.mgukDelivery
			mguhModeLast = car.mguhChargingBatteries
		else
			socModeLast = systemMessageOverride(
				socModeSection,
				"SOC-Mode: " .. string.format("%.0f%%", car.mgukRecovery * 10),
				"",
				car.mgukRecovery,
				socModeLast
			)

			deployModeLast = systemMessageOverride(
				deployModeSection,
				"Deploy-Mode: " .. ac.getMGUKDeliveryName(car.index, car.mgukDelivery),
				"",
				car.mgukDelivery,
				deployModeLast
			)

			mguhModeLast = systemMessageOverride(
				mguhModeSection,
				"MGUH-Mode: " .. (car.mguhChargingBatteries and "Recharge On" or "Recharge Off"),
				"",
				car.mguhChargingBatteries,
				mguhModeLast
			)
		end
	end
...
local MGUK = function()
	local batterySizeKJ = 3570
	local maxKJ = car.kersMaxKJ
	local trackLength = sim.trackLengthM == 0 and 50000 or sim.trackLengthM
	local recoveredEnergy = 0
	local lapCount = 0
	local energyDeficit = 0
	local reduction = 0.5
	local targetSoc = car.mgukRecovery

	return function()
		local trackPosition = ac.worldCoordinateToTrackProgress(car.position)
		local targetKJM = maxKJ * reduction / trackLength
		local currentKJ = car.kersCurrentKJ
		local currentKJM = currentKJ / (trackLength * trackPosition)
		local delta = targetKJM - currentKJM

		recoveredEnergy = (car.kersCharge - 1) * batterySizeKJ + currentKJ
		if car.lapCount ~= lapCount then
			if recoveredEnergy - energyDeficit < 0 then
				ac.setMessage(
					"Energy deficit is: ",
					math.floor(recoveredEnergy - energyDeficit)
						.. " KJ"
						.. " -- LAPS: "
						.. math.floor(10 * car.kersCharge * batterySizeKJ / (-recoveredEnergy / car.lapCount)) / 10
				)
			end

			if debug.mguk then
				ac.debug("_lap." .. car.lapCount .. ".deficit", recoveredEnergy - energyDeficit)
			end

			energyDeficit = recoveredEnergy
			lapCount = car.lapCount
		end

		reduction = math.clamp(car.mgukRecovery / 10 + (car.kersCharge - car.mgukRecovery / 10) * 15, 0, 1)

		if debug then
			ac.debug("ers.mguk.delivery", ac.getMGUKDeliveryName(car.index, car.mgukDelivery))
			ac.debug("ers.KJM.current", currentKJM)
			ac.debug("ers.KJM.target", targetKJM)
			ac.debug("ers.KJM.delta", delta)
			ac.debug("ers.kersInput", car.kersInput)
			ac.debug("ers.kersCharge", string.format("%.1f %%", car.kersCharge * 100))
			ac.debug("ers.KJ.delta", math.floor(car.kersCurrentKJ) - math.floor(recoveredEnergy - energyDeficit))
			ac.debug("ers.KJ.deployed", math.floor(car.kersCurrentKJ))
			ac.debug("ers.KJ.recovered", math.floor(recoveredEnergy - energyDeficit))
			ac.debug("ers.reduction", reduction)
		end

		data.controllerInputs[3] = delta
	end
end

local MGUH = function()
	local engineINI = ac.INIConfig.carData(car.index, "engine.ini")
	local lagUP = engineINI:get("TURBO_0", "LAG_UP", 0.992)
	local lagDN = engineINI:get("TURBO_0", "LAG_DN", 0.998)
	local referenceRPM = engineINI:get("TURBO_0", "REFERENCE_RPM", 4000)
	local tgamma = engineINI:get("TURBO_0", "GAMMA", 0.3)
	local mguhTurboLag = false
	local heatTorqueSharePercentage = 1
	local mguhChargingBatteriesForce = 1

	return function()
		if data.rpm > 10500 then
			mguhChargingBatteriesForce = 1
		else
			mguhChargingBatteriesForce = 0
		end

		if car.mguhChargingBatteries and not mguhTurboLag then
			ac.setTurboExtras(0, lagUP + 0.003, lagDN - 0.0055, referenceRPM + 550, tgamma + 0.129)
			mguhTurboLag = true
		elseif mguhTurboLag then
			ac.setTurboExtras(0, lagUP, lagDN, referenceRPM, tgamma)
			mguhTurboLag = true
		end

		if car.mguhChargingBatteries then
			heatTorqueSharePercentage = 0.78
		else
			heatTorqueSharePercentage = 1
		end

		data.controllerInputs[4] = heatTorqueSharePercentage
		data.controllerInputs[5] = mguhChargingBatteriesForce

		if debug.mguh then
			ac.debug("ers.mguh.mode", car.mguhChargingBatteries and "Batteries" or "Engine")
			ac.debug("ers.mguh.torqShare", tostring((1 - heatTorqueSharePercentage) * 100) .. "%")
		end
	end
end
